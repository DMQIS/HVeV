...
def Pulse2(x, tau1, tau2):
    # normalized s.t. integrates to 1
    return (np.exp(-x/tau1)-np.exp(-x/tau2))/(tau1-tau2)*np.heaviside(x,1)


class loader:
    """
    Class for loading data.
    Most part of the code are not actually used, because there is already the rawIO lib. Originally I was trying to write something for Animal data. 
    """
    ...
    def ADC2uA_NEXUSR7(self,x):
        return x/2**16 *8.192*1e6 / 4/2100/14
    ...
    def fileIO(self,filename,data_type):
        if data_type=="NEXUS":
            ...
            self.data=dict()
            for ch in data:
                self.data[ch] = np.array([(-i+32768).astype(np.int16) for i in data[ch]])
...
class trigger:
    def __init__(self, data, channel_config, trigger_channels, trigger_threshold, TTL_THRESHOLD=300, INRUN_RANDOM=2,
        ...
        trigger_type: int or list of int
                0 - threshold trigger
        ...
        self.data=data
        self.channel_config = cp.copy(channel_config) # list all channels 
        self.trigger_channels = cp.copy(trigger_channels)
        ...
        try:
            ...
            self.MIDAS_DATA = True
            self.ANIMAL_DATA=False
        except:
            self.MIDAS_DATA = False
            try:
                self.dVdI = data["dVdI"]
                self.ANIMAL_DATA=True
            except:
                self.ANIMAL_DATA=False
        ...
        # Force TTL channel to use level trigger.
        ...
        if self.trigger_type[0]!=2:
            if type(trigger_threshold) is list:
                # If trigger_threshold is given in number of sigmas, set the threshold
                if type(trigger_threshold[0]) is str:
                    self.trigger_threshold_sigma = [float(i.split("sigma")[0]) for i in trigger_threshold]
                    self.trigger_threshold = self._set_threshold(self.data_filtered, self.trigger_channels,self.trigger_threshold_sigma)
                    print(f"  Set threshold to {self.trigger_threshold_sigma} sigma, unit in ADC, ",self.trigger_threshold)
                # Use the given threshold
                else:
                    self.trigger_threshold = trigger_threshold       
                    print("  Using default threshold, unit in ADC, ",self.trigger_threshold)
            # Force TTL channel to default threshold
            for i, ch in enumerate(self.trigger_channels):
                if ch == "TTL":
                    self.trigger_threshold[i]=TTL_THRESHOLD
        else:
    ...
    def _set_threshold(self, data, trigger_channels, trigger_threshold_sigma):
        histograms = dict()
        trigger_threshold=[]
        for i,ch in enumerate(trigger_channels):
            # Median is a rough estimation of baseline
            y = data[ch][0] #-np.median(data[ch][0])   
            # h=rh.thist(y, bins=400, range=(min(y),max(y)), name="", title="")
            # h.Fit('gaus',"Q");
            # sigma=h.GetFunction('gaus').GetParameters()[2]
            # Try not to use ROOT...
            try:
                popt,pcov,x,y = self._gaus_fit(data[ch][0][self.pre_trig:-self.post_trig],bins=400)
                sigma=popt[2]
                histograms[ch]=[x,y]
            except:
                sigma = np.std(data[ch][0])
                sigma=20
                print("Fail to fit for channel",ch)

            trigger_threshold.append(trigger_threshold_sigma[i]*np.abs(sigma))
            ...
            for i,ch in enumerate(self.trigger_channels):
                ...
                if self.trigger_type[i] in [0,1]:
                    ...
                    trigger_points, trigger_amps, trigger_width = self._threshold_trigger(trace, trig_th, rising_edge=rising_edge,
                           align_max=align_max_temp, deactivate_th=None, peak_search_window_limit=self.post_trig, trigger_offset=trigger_offset)
                ...
                if self.trigger_type[i] ==2 or (self.INRUN_RANDOM and i==0):
                ...
                if self.MIDAS_DATA:
                    ...
                    trigger_result["filename_idx"] = np.append(trigger_result["filename_idx"],[int(self.data["filename"].split("_")[-1].split(".")[0][1:])]*n_triggers)
...
def sortByExt(files):
    numbers=list()
    for f in files:
        f_idx=re.sub("[^0-9]", "", f.split('_')[-1].split('.')[0][:])
        numbers.append(int(f_idx))
    return np.array(files)[np.argsort(numbers)].tolist()
...
fallback_PSD_fname = "/gpfs/slac/staas/fs1/g/supercdms/users/TUNL_ana/users/tom/R7_processing/PSDs_25201109_212327.joblib"
def getPSD(Traces, Fs=625000, weight=1, channels=None, autocuts=True):
    ...
    for ch in Channels:
        if ch =="TTL":
            continue
        print(ch)
        noiseResults = Noise.processNoise(Traces[ch]*1e-6*weight, traceGain = 1., makePlots=False, autoCut=autocuts, fs=Fs, fileStr='_', saveResults=False, useCorrectAveraging=True)
