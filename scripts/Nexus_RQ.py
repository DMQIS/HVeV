...
class RQ: 
    def __init__(self, OFP_WINDOW=250, ENABLE_OFP=True, OFL_MAX_DELAY=25, MAX_CHISQ_FREQ_BIN=-1, Fs=625000, Pretrig=4096, Posttrig=4096,
                 BaseLength=4086, TailLength=3096, ShortChi2Length=200, PlateauDelay=100, PlateauLength=100,
                 UseFilterForRQs=False, CutoffFrequenciesForRQs=None, FilterOrderForRQs=10, WindowForPulseRQs=100):
        self.OFP_WINDOW = OFP_WINDOW
        self.ENABLE_OFP = ENABLE_OFP
...
#             res[key]=res_2[key]
    ...
    def _make_filter_kernel_OF(self, filter_kernels, PSD, OF_LPF_dict=-1):
        ...
        if type(PSD) is dict and type(filter_kernels) is dict:
            ...
            for key in PSD:
                ...
                OF_norm = np.real(OF.dot(trigTemplate_fft))
                ...
                Js[key]=J
                # The variance of noise in time domain. Useful when calculating time domain chi2
                filter_kernels_varPSD[key] = (irfft(PSD_template**2)*self.Fs)[0]
                if OF_LPF>0:
                    psd_lim = np.copy(PSD_template)
                    psd_lim[OF_LPF:]=0
                    filter_kernels_varPSD_limited[key] = (irfft(psd_lim**2)*self.Fs)[0]
                else:
    ...
    def _run_OF_ch(self, trace, ch, OF_LPF=-1, pileup_window=250):  
        """
        Did not use the scdmsPyTools, trying to not compute the filter coefficient every time. 
        """
        OFL_max_delay = self.OFL_MAX_DELAY
        Ns = float(len(trace))
        dt = 1/self.Fs
        chiScale=4*dt/(Ns**2)

        # 1. Filter
        filter_kernel_freq=self.filter_kernels_normed_freq[ch]
        OF_norm_for_chi2_rq = self.OF_norm_for_chi2_rq[ch]*1e12
        OF_norm = self.OF_norm[ch]*1e12
        Tf = self.trigTemplates_freq[ch]
        # 2. PSD
        J = self.Js[ch]*1e-12 # uA^2
        # 3. trace
        trace_freq = rfft(trace) # uA
        # 4. filtered trace
        trace_filt = filter_kernel_freq*trace_freq
        
        # 5. LPF
        chi2LPF_SF = 1.
        if (OF_LPF > 0) and (OF_LPF < len(J)):
            J[OF_LPF:]=np.maximum(J[OF_LPF:],J[OF_LPF])
            trace_freq[OF_LPF:] = 0
            chi2LPF_SF = float(OF_LPF)/len(J)
        chiScale = chiScale/chi2LPF_SF
        
        if self.MAX_CHISQ_FREQ_BIN>=0 and (self.MAX_CHISQ_FREQ_BIN<OF_LPF or OF_LPF<0):
            chi2LPF_SF = float(self.MAX_CHISQ_FREQ_BIN)/len(J)
        chiScale_for_chi2_rq = chiScale/chi2LPF_SF

        # --------------------
        # 1. OF without delay
        A0 = np.real(np.sum(trace_filt))
        #signal part of chi-square
        chi0 = np.real(np.dot(trace_freq.conjugate()/J,trace_freq))
        chi0_for_chi2_rq = chi0 if self.MAX_CHISQ_FREQ_BIN<0 else np.real(np.sum((trace_freq.conjugate()*trace_freq/J)[:self.MAX_CHISQ_FREQ_BIN]))
        # chi-square
        Xr0 = (chi0 - (A0**2)*OF_norm)*chiScale
        Xr0_for_chi2_rq = (chi0_for_chi2_rq - (A0**2)*OF_norm_for_chi2_rq)*chiScale_for_chi2_rq

        # --------------------        
        # 2. OF with delay
        At = irfft(trace_filt)*Ns/2
        #fitting part of chi-square
        chit = (At**2)*OF_norm
        #sum parts of chi-square
        chi = (chi0 - chit)*chiScale
        chi_for_chi2_rq = (chi0_for_chi2_rq - (At**2)*OF_norm_for_chi2_rq)*chiScale_for_chi2_rq
        #find time of best-fit
        # 2.1 No limit on time delay
        bInd = np.argmin(chi)
        A = At[bInd]
        Xr = chi[bInd]
        Xr_for_chi2_rq = chi_for_chi2_rq[bInd]
        t0=bInd*dt
        # 2.2 Limit time delay
        bInd = np.argmin(np.concatenate((chi[-OFL_max_delay:],chi[:OFL_max_delay])))
        bInd -= OFL_max_delay
        AL = At[bInd]
        XrL = chi[bInd]
        XrL_for_chi2_rq = chi_for_chi2_rq[bInd]
        t0L=bInd*dt
        ...
        if ("total" in ch or "Total" in ch) and self.ENABLE_OFP:
            ...
            NoisePSD = np.copy(self.PSDs[ch])*1e-6 # Turn unit into uA
            Signal = trace
            ...
            t1,t2 = np.rint(t1*self.Fs), np.rint(t2*self.Fs) # Turn the unit into samples and centered at 0
        else:
            A1s,A2s,t1,t2,Xrp,Xrp_for_chi2_rq =0,0,0,0,0,0
        ...
        if OF_LPF>0:
            ...
            trace_lpf = trace
        return A0,Xr0,Xr0_for_chi2_rq,  A,Xr,Xr_for_chi2_rq,t0,  trace_lpf,  A1s,A2s,t1,t2,Xrp,Xrp_for_chi2_rq,  AL,XrL,XrL_for_chi2_rq,t0L
    ...
    def _run_OF(self, Traces):
        ...
        OF_results=dict()
        tlength = self.Pretrig+self.Posttrig
        tlength_2=tlength/self.Fs/2
        RQlist=["OF0", "OF0_chi2", "OF0_chi2_full", "OF0_chi2time", "OF", "OF_chi2", "OF_chi2_full", "OF_time",
                "OFL", "OFL_chi2", "OFL_chi2_full", "OFL_time", "OF_chi2time", "OF0_chi2timeFiltered", "OF0_chi2timeShort",
        ...
        for ch in Traces:
            ...
            varPSD = self.filter_kernels_varPSD[ch]*1e-12 # Turn unit into uA
            varPSDlim = self.filter_kernels_varPSD_limited[ch]*1e-12 # Turn unit into uA
            ...
            for trace in Traces[ch]:
                ...
                A0,Xr0,Xr0_for_chi2_rq,  A,Xr,Xr_for_chi2_rq,t0,  trace_lpf,  A1s,A2s,t1,t2,Xrp,Xrp_for_chi2_rq,  AL,XrL,XrL_for_chi2_rq,t0L =\
                    self._run_OF_ch(trace*self.ADC2uA, ch, OF_LPF=self.OF_LPF_dict[ch], pileup_window=self.OFP_WINDOW)
                
                OF_results["OF0_"+ch]      =np.append(OF_results["OF0_"+ch],A0)
                OF_results["OF0_chi2_full_"+ch] =np.append(OF_results["OF0_chi2_full_"+ch],Xr0)
                OF_results["OF0_chi2_"+ch] =np.append(OF_results["OF0_chi2_"+ch],Xr0_for_chi2_rq)
                OF_results["OF_"+ch]       =np.append(OF_results["OF_"+ch],A)
                OF_results["OF_chi2_full_"+ch]  =np.append(OF_results["OF_chi2_full_"+ch],Xr)
                OF_results["OF_chi2_"+ch]  =np.append(OF_results["OF_chi2_"+ch],Xr_for_chi2_rq)
                OF_results["OF_time_"+ch]  =np.append(OF_results["OF_time_"+ch], np.rint(self.Fs*((t0+tlength_2)%(2*tlength_2)-tlength_2))) # unit in samples
                OF_results["OFL_"+ch]      =np.append(OF_results["OFL_"+ch],AL)
                OF_results["OFL_chi2_full_"+ch] =np.append(OF_results["OFL_chi2_full_"+ch],XrL)
                OF_results["OFL_chi2_"+ch] =np.append(OF_results["OFL_chi2_"+ch],XrL_for_chi2_rq)
                OF_results["OFL_time_"+ch] =np.append(OF_results["OFL_time_"+ch], np.rint(self.Fs*((t0L+tlength_2)%(2*tlength_2)-tlength_2))) # unit in samples
                
                if "total" in ch or "Total" in ch:
                    OF_results["OFP1_"+ch]     =np.append(OF_results["OFP1_"+ch],A1s)
                    OF_results["OFP2_"+ch]     =np.append(OF_results["OFP2_"+ch],A2s)
                    OF_results["OFP1_time_"+ch]=np.append(OF_results["OFP1_time_"+ch],t1) # unit in samples
                    OF_results["OFP2_time_"+ch]=np.append(OF_results["OFP2_time_"+ch],t2) # unit in samples
                    OF_results["OFP_chi2_full_"+ch] =np.append(OF_results["OFP_chi2_full_"+ch],Xrp)
                    OF_results["OFP_chi2_"+ch] =np.append(OF_results["OFP_chi2_"+ch],Xrp_for_chi2_rq)
                
                #compute no-delay time domain chi-square
                fit = A0*chanTemplate
                err = trace*self.ADC2uA - fit
                xt = np.var(err)/varPSD*2
                OF_results['OF0_chi2time_'+ch].append(xt)
                err = trace_lpf - fit
                xt = np.var(err)/varPSDlim*2
                OF_results['OF0_chi2timeFiltered_'+ch].append(xt)
                xt = np.var(err[pretrig:pretrig+self.ShortChi2Length])/varPSDlim*2
                OF_results['OF0_chi2timeShort_'+ch].append(xt)
                ...
                if self.UseFilterForRQs:
                    ...
                    filtered_trace = butter_filter(trace*self.ADC2uA, self.CutoffFrequenciesForRQs[ch], self.FilterOrderForRQs, fs=self.Fs)
                
                baseline = np.mean(filtered_trace[:base_length])
                x_time = np.arange(len(trace))
                S = filtered_trace - baseline
                threshold = saturation_amplitude[ch]*MF_threshold
